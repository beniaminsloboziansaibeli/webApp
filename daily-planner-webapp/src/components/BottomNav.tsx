...
export default function BottomNav({ active='today', onNavigate }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[92%] max-w-xl rounded-2xl px-3 py-2 flex justify-between items-center card-glass" style={{ backdropFilter: 'blur(10px)' }}>
      {items.map(it => (
        <motion.button key={it.key} onClick={() => onNavigate?.(it.key)} className={`flex flex-col items-center gap-1 px-4 py-2 rounded-lg min-w-[64px] ${active === it.key ? 'bg-primary/10 text-primary' : 'text-muted'}`} whileTap="press" variants={press} initial="rest">
          {it.icon}
