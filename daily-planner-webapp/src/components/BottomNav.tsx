...
export default function BottomNav({ active='today', onNavigate }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[92%] max-w-xl rounded-2xl px-3 py-2 flex justify-between items-center card-glass" style={{ backdropFilter: 'blur(10px)' }}>
      {items.map(it => (
        <motion.button key={it.key} onClick={() => onNavigate?.(it.key)} className={`flex flex-col items-center gap-1 px-4 py-2 rounded-lg min-w-[64px] ${active === it.key ? 'bg-primary/10 text-primary shadow-[0_6px_20px_rgba(0,183,255,0.08)]' : 'text-muted'}`} whileTap={reduced ? undefined : 'press'} variants={reduced ? undefined : press} initial={reduced ? undefined : 'rest'}>
          {it.icon}
          <span className="text-xs">{t(it.label)}</span>
        </motion.button>
      ))}
    </nav>
  )
