...
const CompletionModal: React.FC<{ open: boolean; onClose: () => void; message?: string }> = ({ open, onClose, message }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <motion.div initial={reduced ? undefined : { scale: 0.86, opacity: 0 }} animate={reduced ? undefined : { scale: 1, opacity: 1 }} transition={reduced ? undefined : { type: 'spring', stiffness: 300, damping: 24 }} className="z-10 w-11/12 max-w-sm card-glass p-6 text-center">
